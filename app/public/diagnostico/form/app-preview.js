...
async function showResults(){
  ...
  document.getElementById('results').innerHTML=`
    ...
    <div class="exec-card">
      ...
      ${D.responsibleName?`<div style="font-size:12px;color:var(--mu);margin-bottom:10px">${D.responsibleName}${D.responsibleRole?' · '+D.responsibleRole:''}</div>`:''}
      <div class="kpis">
      ...
      ${(D.shareHouse>0||D.shareParcerias>0)?`
      <div style="display:flex;gap:8px;margin:10px 0">
        ${D.hasHouse&&D.shareHouse>0?`<div style="flex:${D.shareHouse};background:rgba(245,158,11,0.12);border:1px solid rgba(245,158,11,0.2);border-radius:8px;padding:6px 10px;text-align:center"><div style="font-size:10px;color:var(--am);text-transform:uppercase;letter-spacing:.05em">House</div><div style="font-size:16px;font-weight:700;color:var(--am)">${D.shareHouse}%</div></div>`:''}
        ${D.shareParcerias>0?`<div style="flex:${D.shareParcerias};background:rgba(232,57,42,0.08);border:1px solid rgba(232,57,42,0.15);border-radius:8px;padding:6px 10px;text-align:center"><div style="font-size:10px;color:var(--red);text-transform:uppercase;letter-spacing:.05em">Parcerias</div><div style="font-size:16px;font-weight:700;color:var(--red)">${D.shareParcerias}%</div></div>`:''}
      </div>`:''}
      <div style="display:flex;gap:8px;margin-bottom:10px;flex-wrap:wrap">
        <span style="font-size:11px;padding:4px 10px;border-radius:6px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.08);color:var(--mu)">Metas: <strong style="color:${D.meetingGoals==='yes'?'var(--gr)':D.meetingGoals==='partial'?'var(--am)':'var(--red)'}">${D.meetingGoals==='yes'?'Sim':D.meetingGoals==='partial'?'Parcialmente':'Não'}</strong></span>
        <span style="font-size:11px;padding:4px 10px;border-radius:6px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.08);color:var(--mu)">Exclusividade: <strong style="color:#fff">${{exclusive:'Exclusivos','non-exclusive':'Não exclusivos',mixed:'Misto'}[D.brokersExclusivity]||'—'}</strong></span>
        ${D.hasImob&&D.numImobiliarias?`<span style="font-size:11px;padding:4px 10px;border-radius:6px;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.08);color:var(--mu)">Imobiliárias: <strong style="color:#fff">${D.numImobiliarias}</strong></span>`:''}
      </div>
      <div class="verdict">
    ...
    <div class="db"><div class="dh" onclick="tog(this)"><h3>4 · 3 Rotas Estratégicas</h3><div class="dhr"><span class="dtag">Como atingir a meta</span><span class="chev">▾</span></div></div>
      <div class="dbody">
        ...
        ${D.challengesText?`<div style="margin-top:14px;padding:12px 16px;background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.06);border-radius:10px"><div style="font-size:10px;color:var(--mu);text-transform:uppercase;letter-spacing:.07em;margin-bottom:6px">Contexto do cliente</div><div style="font-size:13px;color:#ccc;line-height:1.6">${D.challengesText}</div></div>`:''}
        ${D.testedActions?`<div style="margin-top:10px;padding:12px 16px;background:rgba(245,158,11,0.04);border:1px solid rgba(245,158,11,0.1);border-radius:10px"><div style="font-size:10px;color:var(--am);text-transform:uppercase;letter-spacing:.07em;margin-bottom:6px">Ações já testadas</div><div style="font-size:13px;color:#ccc;line-height:1.6">${D.testedResults||'Sim, mas sem resultados descritos.'}</div></div>`:''}
      </div>
    </div>
    ...
    <div class="db"><div class="dh" onclick="tog(this)"><h3>5 · Organograma e Fluxos · Canal Parcerias</h3><div class="dhr"><span class="dtag">Sem DWV vs. Com DWV</span><span class="chev">▾</span></div></div>
      <div class="dbody">
        ...
        ${CARGOS_CANAL.some(c=>D.cargos[c.id]?.existe)?`
        <div style="margin-top:16px">
          <div style="font-size:11px;color:var(--mu);text-transform:uppercase;letter-spacing:.07em;margin-bottom:10px">Detalhamento por Cargo</div>
          <table style="width:100%;border-collapse:collapse;font-size:12px">
            <tr style="border-bottom:1px solid rgba(255,255,255,0.08)">
              <th style="text-align:left;padding:8px;color:var(--mu);font-weight:600">Cargo</th>
              <th style="text-align:center;padding:8px;color:var(--mu);font-weight:600">Qtd</th>
              <th style="text-align:left;padding:8px;color:var(--mu);font-weight:600">KPI / Métrica</th>
              <th style="text-align:left;padding:8px;color:var(--mu);font-weight:600">Atividades</th>
            </tr>
            ${CARGOS_CANAL.filter(c=>D.cargos[c.id]?.existe).map(c=>`
            <tr style="border-bottom:1px solid rgba(255,255,255,0.04)">
              <td style="padding:8px;color:#fff;font-weight:500">${c.nome}</td>
              <td style="padding:8px;text-align:center;color:var(--am);font-weight:700">${D.cargos[c.id].qtd}</td>
              <td style="padding:8px;color:#ccc">${D.cargos[c.id].kpi||'<span style="color:var(--mu)">—</span>'}</td>
              <td style="padding:8px;color:#aaa;font-size:11px;max-width:300px">${D.cargos[c.id].atividades?D.cargos[c.id].atividades.substring(0,120)+(D.cargos[c.id].atividades.length>120?'...':''):'<span style="color:var(--mu)">—</span>'}</td>
            </tr>`).join('')}
          </table>
        </div>`:''}

        ${D.brokerSegmentation&&D.brokerSegmentation!=='nao'?`
        <div style="margin-top:14px;padding:12px 16px;background:rgba(139,92,246,0.04);border:1px solid rgba(139,92,246,0.1);border-radius:10px">
          <div style="font-size:10px;color:#a78bfa;text-transform:uppercase;letter-spacing:.07em;margin-bottom:6px">Segmentação da Base</div>
          <div style="font-size:13px;color:#ccc">${{nao_gostaria:'Não possui, mas gostaria',sim_parcial:'Sim, parcialmente',sim_total:'Sim, totalmente'}[D.brokerSegmentation]||D.brokerSegmentation}</div>
          ${D.brokerSegDescritivo?`<div style="font-size:12px;color:#999;margin-top:6px;line-height:1.5">${D.brokerSegDescritivo}</div>`:''}
        </div>`:''}
      </div>
    </div>
    ...
    <div class="db"><div class="dh" onclick="tog(this)"><h3>6 · Tecnologia e Ferramentas</h3><div class="dhr"><span class="dtag">${r.nTools} desconectadas</span><span class="chev">▾</span></div></div>
      <div class="dbody">
        ${!D.hasCRM?'<div class="cl" style="margin-bottom:12px">Sem CRM voltado para parceiros. Canal opera sem rastreamento de ações por corretor.</div>':`<div style="font-size:13px;color:var(--mu);margin-bottom:12px">CRM de parceiros: <strong style="color:#fff">${D.crmName||'não especificado'}</strong></div>`}
        ${D.crmContratoNome?`<div style="font-size:13px;color:var(--mu);margin-bottom:12px">CRM de contratos: <strong style="color:#fff">${D.crmContratoNome}</strong> — focado em contratos, não em gestão de corretores.</div>`:''}
    ...
    <div class="db"><div class="dh" onclick="tog(this)"><h3>7 · Retorno sobre Investimento DWV</h3><div class="dhr"><span class="dtag g">${fmtN(r.corretoresAdicionar)} corretores · ${fmt(r.retornoTotal)}</span><span class="chev">▾</span></div></div>
      <div class="dbody">
        ...
        <div class="cl g">
          ${rptLabels.length>0?`Relatórios desejados pelo cliente — <strong>${rptLabels.join(', ')}</strong> — todos disponíveis na plataforma DWV.<br/>`:''}
          ${D.desiredReportsDescritivo?`<div style="font-size:12px;color:#999;margin-top:4px;font-style:italic">"${D.desiredReportsDescritivo}"</div><br/>`:''}
